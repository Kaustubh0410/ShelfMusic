...
The project uses a **Spotify-style audio-features dataset**. The schema mirrors
the public Kaggle dataset
[*Spotify Tracks Dataset*](https://www.kaggle.com/datasets/maharshipandya/spotify-tracks-dataset)
(track/artist/genre metadata + audio features such as `danceability`, `energy`,
`valence`, `acousticness`, `tempo`, `loudness`, etc.).

To keep the repository **plug-and-play with no external downloads or API keys**,
`data/dataset.csv` is generated by `data/generate_dataset.py`, which synthesizes
genre-consistent audio features (each genre has a realistic feature profile plus
noise). This produces meaningful, explainable recommendations out of the box.

**Using the real Kaggle data instead:** download the dataset from the link
above, rename it to `dataset.csv` with the same columns, and drop it into
`data/`. The backend seeds whatever `data/dataset.csv` contains on first run.
...
│   └── generate_dataset.py    # regenerates dataset.csv
├── backend/
│   ├── Dockerfile
│   ├── requirements.txt
│   ├── app/
